...
public class MBRspUpLoadPic implements MsgPara {

	public short nResult; //是否成功, 0 --- 表示成功
	public String iMsg; //信息
	public int iUploadSize; //已经上传的大小
	public String 	iFileName;		//系统保存的文件名


	/**
	 * construct
	 */
	public MBRspUpLoadPic() {
		nResult = -1;
		iMsg = "";
		iUploadSize = 0;
		iFileName = "";
	...
	public int Decode(byte[] aInBuffer, short aInLength) {
		...
		iUploadSize = Temp.getInt();
		int filelen = Temp.getShort();
		if(filelen > 0) {
			byte[] filename = new byte[filelen];
			Temp.get(filename);
			iFileName = new String(filename);
			filename = null;
		}
		return Temp.position();
	}
	...
	public String toString() {
		return "MBRspUpLoadPic [nResult=" + nResult + ", iMsg=" + iMsg
				+ ", iUploadSize=" + iUploadSize + ", iFileName=" + iFileName
				+ "]";
	}
}
