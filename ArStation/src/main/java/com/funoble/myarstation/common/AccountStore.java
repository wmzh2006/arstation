...
public class AccountStore
{
	...
	public void open()
	{
		Tools.debug("AccountStore::open() start");
		InputStream in = null;
		DataInputStream din = null;
		try
		{
			in = context.openFileInput(this.fileName);
			din = new DataInputStream(in);
			readStream(din);
			in.close();
			din.close();
			Tools.debug("AccountStore::open() end");
		}
		catch (Exception e)
		{
			// Toast
			...
			// Toast.LENGTH_LONG)
			// .show();
			Tools.debug("AccountStore::open() error1!!!");
			try
			{
				if (in != null)
				{
			...
			{
				Tools.debug("AccountStore::open() error2!!!");
			}
		}
	}
	...
	public void commit()
	{
		Tools.debug("AccountStore::commit() start");
		OutputStream out = null;
		DataOutputStream dout = null;
		try
		{
			out = context.openFileOutput(this.fileName, Context.MODE_PRIVATE);
			dout = new DataOutputStream(out);
			writeStream(dout);
			out.close();
			dout.close();
			Tools.debug("AccountStore::commit() end");
		}
		catch (Exception e)
		{
			Tools.debug("AccountStore::commit() error1!!!");
			// Toast
		...
		{
			Tools.debug("AccountStore::commit() error2!!!");
			try
			{
				if (out != null)
				{
			...
			{
				Tools.debug("AccountStore::commit() error3!!!");
			}
		}
	}
